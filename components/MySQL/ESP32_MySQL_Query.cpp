...
bool ESP32_MySQL_Query::execute(const char *query, bool progmem)
{
  ...
  if (conn->largest_buffer_size < query_len + COMMAND_HEADER_LEN)
  {
    if (conn->largest_buffer_size == 0)
    {
      ...
      conn->buffer = (char *)malloc(conn->largest_buffer_size);
    }
    else
    {
      ...
      conn->buffer = (char *)realloc(conn->buffer, conn->largest_buffer_size);
    }
  }
  /*else
  {
...
/*
  ...
  query_len[in]   Number of chars in the query string
...
bool ESP32_MySQL_Query::execute_query(const int &query_len)
{
  ...
  conn->buffer[3] = char(0x00);
  conn->buffer[4] = char(0x03); // command packet

  // Send the query
  // ESP32_MYSQL_LOGDEBUG1("ESP32_MySQL_Query::execute_query: query = ", (char *) &conn->buffer[COMMAND_HEADER_LEN] );

  conn->client->write(conn->buffer, query_len + COMMAND_HEADER_LEN);
  conn->client->flush();
...
void ESP32_MySQL_Query::show_results()
{
  ...
  for (int f = 0; f < columns.num_fields; f++)
  {
    ...
    if (f < columns.num_fields - 1)
      continue; // ESP32_MYSQL_LOGERROR0(',');
  }
...
/*
  ...
  NOTICE: Failing to call this method after calling get_columns()
          ...
          the size of the combined column names (chars).
*/
...
/*
  ...
  NOTICE: You must call this method at least once after you
          ...
          of the length of values and one char for each max cols.
*/
...
char *ESP32_MySQL_Query::read_string(int *offset)
{
  ...
  int len_chars = conn->get_lcb_len(conn->buffer[*offset]);
  int len = conn->read_int(*offset, len_chars);

  // ESP32_MYSQL_LOGINFO1("ESP32_MySQL_Query::read_string: offset = ", *offset);
  // ESP32_MYSQL_LOGINFO3("ESP32_MySQL_Query::read_string: len = ", len, "len_chars =", len_chars);
  ...
  if ((len < 251) && (len > 0))
  {
    ...
    strncpy(str, (char *)&conn->buffer[*offset + len_chars], len);
    str[len] = 0x00;
    *offset += len_chars + len;
...
int ESP32_MySQL_Query::get_field(field_struct *fs)
{
  ...
  if (conn->buffer && conn->buffer[4] != ESP32_MYSQL_EOF_PACKET)
  {
    ...
    len_chars = conn->get_lcb_len(4);
    len = conn->read_int(4, len_chars);
    offset = 4 + len_chars + len;
    ...
    len_chars = conn->get_lcb_len(offset);
    len = conn->read_int(offset, len_chars);
    offset += len_chars + len;

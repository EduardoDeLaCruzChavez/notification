...
void Encrypt_SHA1::pad()
{
  ...
  // Append length in the last 8 chars
  addUncounted(0);               // We're only using 32 bit lengths
  addUncounted(0);               // But SHA-1 supports 64 bit lengths
  addUncounted(0);               // So zero pad the top bits
  addUncounted(charCount >> 29); // Shifting to multiply by 8
  addUncounted(charCount >> 21); // as SHA-1 supports bitstreams as well as
  addUncounted(charCount >> 13); // char.
  addUncounted(charCount >> 5);
  addUncounted(charCount << 3);
}

...
static const char *TAG_SNMP = "SNMP Manager";
...
void vInitSNMP(char *pcIPHost, TYPE_SNMP_SESION *ptSesion)
{
    ...
    ESP_LOGD(TAG_SNMP, "Socket created, sending to %s:%d", pcIPHost, PORT);
}
...
int iRecvQuery(TYPE_SNMP_SESION *ptSesion, TYPE_SNMP_QUERY *ptQueryRecv)
{
    ...
    if (iLen > 0)
    {
        ESP_LOGD(TAG_SNMP, "Received %d bytes", iLen);
...
int8_t i8ReadResponse(TYPE_SNMP_QUERY *ptSNMPRes, TYPE_RESPONSE_QUERY *ptQueryRes)
{
    ...
    while (eState != eSNMP_MAX_STATE)
    {
        ...
        case eSNMP_COMUNITY:
        {
            ...
            // ESP_LOGV(TAG_SNMP, "Comunity: %s", acBuffer);
            break;
        }
        case eSNMP_TYPE_PDU:
        {
        ...
        case eSNMP_REQUEST_ID:
        {
            ...
            // ESP_LOGV(TAG_SNMP, "Request ID: %" PRIu32 "", u32Buffer);
            break;
        }
        case eSNMP_ERROR_:
        {
            ...
            u32Buffer = u32GetInteger(ptSNMPRes);
            // ESP_LOGV(TAG_SNMP, "Error: %" PRIu32 "", u32Buffer);
            break;
        }
        case eSNMP_ERROR_INDEX:
        {
            ...
            if (u32Buffer != 0)
            {
                // ESP_LOGV(TAG_SNMP, "Error Index: %" PRIu32 "", u32Buffer);
                return -1;
            }
            break;
        }
...
int8_t i8SnmpGetNext(char *pcIPHost, const char *pcType, const char *pcODI, TYPE_RESPONSE *ptResponse)
{
    ...
    do
    {
        ...
        if (u8Error != 0)
        {
            ESP_LOGD(TAG_SNMP, "Error %d", (int)u8Error);
            break;
        }
